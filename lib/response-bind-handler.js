...
function responseBindHandler(ctx, bind, response) {
  ...
  if (!bind) bind = new Bind({ where: 'body' });
  if (bind.where === 'body') {
    if (bind.name) {
      if (typeof ctx.response.body !== 'object') ctx.response.body = {};
      ctx.response.body[bind.name] = response.body;
    } else ctx.response.body = response.body;
    ctx.response.headers = response.headers;
  } else if (bind.where === 'headers') {
    if (bind.name) ctx.set(bind.name, response.body);
    else ctx.set(response.body);
